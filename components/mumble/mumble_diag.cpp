...
void mumble_log_hex(const char *tag, const char *prefix, const uint8_t *data, size_t len, size_t max_bytes) {
...
void mumble_diag_run_boot() {
  ...
  ESP_LOGI(TAG, "  0x0A0B0C0D in memory: %02x %02x %02x %02x", (unsigned)val.u8[0], (unsigned)val.u8[1],
           (unsigned)val.u8[2], (unsigned)val.u8[3]);
  ...
  if (inet_aton("172.20.13.17", &ia) != 0) {
    ESP_LOGI(TAG, "  inet_aton(172.20.13.17) -> s_addr=0x%08lx", (unsigned long)(uint32_t)ia.s_addr);
    uint8_t *p = (uint8_t *)&ia.s_addr;
    ESP_LOGI(TAG, "    Bytes in s_addr: %02x %02x %02x %02x", (unsigned)p[0], (unsigned)p[1], (unsigned)p[2],
             (unsigned)p[3]);
    uint32_t ho = ntohl(ia.s_addr);
    ESP_LOGI(TAG, "    As dotted quad (ntohl): %lu.%lu.%lu.%lu", (unsigned long)((ho >> 24) & 0xFF),
             (unsigned long)((ho >> 16) & 0xFF), (unsigned long)((ho >> 8) & 0xFF), (unsigned long)(ho & 0xFF));
  }
  ...
  ESP_LOGI(TAG, "  For network order: htonl(s_addr) -> %lu.%lu.%lu.%lu", (unsigned long)((ho2 >> 24) & 0xFF),
           (unsigned long)((ho2 >> 16) & 0xFF), (unsigned long)((ho2 >> 8) & 0xFF), (unsigned long)(ho2 & 0xFF));
