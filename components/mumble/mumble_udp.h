...
class MumbleUdp {
 public:
  ...
  static constexpr uint32_t UDP_PING_INITIAL_COUNT = 5;
  ...
  ~MumbleUdp() {
    stop();
    if (udp_ != nullptr) {
      mumble_free_udp_socket(udp_);
      udp_ = nullptr;
    }
  }

  /** Start UDP. peer_ip in network byte order (0=resolve). */
  bool start(const std::string &server, uint16_t port, uint32_t peer_ip = 0);
  void stop();
