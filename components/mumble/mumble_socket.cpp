...
public:
  ...
  bool connect(const char *host, uint16_t port, uint32_t timeout_ms) override {
    ...
    BaseType_t ok = xTaskCreate(tls_connect_task, "tls_conn", 10240, &task_params_, 5, &connect_task_);
    if (ok != pdPASS) {
...
public:
  ...
  size_t write(const uint8_t *buf, size_t len) override {
    if (send_pos_ + len > sizeof(send_buf_))
      len = sizeof(send_buf_) - send_pos_;
    memcpy(send_buf_ + send_pos_, buf, len);
    send_pos_ += len;
    return len;
  }
  bool end_packet() override {
    if (conn_ == nullptr || send_pos_ == 0)
      return false;
    ...
    ip_addr_t dst;
    IP4_ADDR(&dst, (uint8_t)(remote_ip_ >> 24), (uint8_t)(remote_ip_ >> 16), (uint8_t)(remote_ip_ >> 8),
             (uint8_t)(remote_ip_ & 0xff));
    ...
    if (!ok) {
      ...
      ESP_LOGW(TAG, "UDP netconn_sendto: target %s:%u len=%zu err=%d", ipbuf, remote_port_, send_pos_, (int)err);
    }
    return ok;
  }
...
TlsClient *mumble_create_tls_client() {
  return new TlsClientEspIdf;
}
UdpSocket *mumble_create_udp_socket() {
  return new UdpSocketNetconn;
}
...
public:
  ...
  int last_error(char *buf, size_t buf_len) override { return client_.lastError(buf, buf_len); }
  uint32_t get_peer_ip() const override {
...
TlsClient *mumble_create_tls_client() {
  return new TlsClientArduino;
}
UdpSocket *mumble_create_udp_socket() {
  return new UdpSocketArduino;
}
void mumble_free_tls_client(TlsClient *c) {
  delete c;
}
